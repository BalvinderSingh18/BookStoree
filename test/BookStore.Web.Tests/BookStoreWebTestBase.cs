...
{
    public abstract class BookStoreWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static BookStoreWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(BookStoreWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<BookStoreDbContext> action)
        {
            using (var context = IocManager.Resolve<BookStoreDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<BookStoreDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<BookStoreDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<BookStoreDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<BookStoreDbContext, Task<T>> func)
        {
            T result;
